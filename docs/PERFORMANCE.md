...
5. incrementally updates target-fanout windows for the affected target
6. incrementally updates the low-context fanout window for the affected target/referrer key
7. incrementally updates the payload-campaign window for the affected campaign family
8. composes the final action and applies transition rules
...
- amortized `O(1)` to `O(target_window)` for target and low-context fanout maintenance
- amortized `O(1)` to `O(campaign_window)` for payload-campaign maintenance on the affected family

That means live cost now scales mainly with the current IP and the current distributed keys, not with the whole retained global history.
...
- large same-target distributed windows
...
- coordinated target fanout on very hot target windows
- low-context fanout on very hot target/referrer windows
