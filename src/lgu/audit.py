...
@lru_cache(maxsize=131072)
def normalized_referer_host(referer: str) -> str:
    parsed = urlparse(referer)
...
@lru_cache(maxsize=131072)
def low_context_referer_key(referer: str) -> str | None:
    if referer == "-":
        return "direct"
    parsed = urlparse(referer)
    path = parsed.path or "/"
    if parsed.query or path != "/":
        return None
    host = normalized_referer_host(referer)
    if not host:
        return "root"
    return f"root:{host}"


def is_low_context_fanout_candidate(row: RequestRow) -> bool:
    return bool(
        row.method == "GET"
        and row.status.startswith(("2", "3"))
        and low_context_referer_key(row.referer) is not None
        and is_content_page_path(row.path)
        and target_profile(row.path) != "root"
    )
...
def detect_low_context_fanout(
    rows: list[RequestRow], args: argparse.Namespace
) -> CoordinatedUAAnalysis:
    degraded = False
    degraded_reason = ""
    ip_totals = Counter(row.ip for row in rows)
    candidate_rows = [
        row
        for row in rows
        if ip_totals[row.ip] <= args.low_context_fanout_max_ip_requests
        and is_low_context_fanout_candidate(row)
    ]
    if not candidate_rows:
        return CoordinatedUAAnalysis(forced_proofs_by_ip={})
    if len(candidate_rows) > args.low_context_fanout_max_rows:
        degraded = True
        degraded_reason = (
            "bounded to most recent "
            f"{args.low_context_fanout_max_rows} candidate rows from {len(candidate_rows)}"
        )
        candidate_rows = heapq.nlargest(
            args.low_context_fanout_max_rows,
            candidate_rows,
            key=lambda row: (row.ts, row.ip, row.path),
        )

    sorted_rows = sorted(candidate_rows, key=lambda row: (row.ts, row.ip, row.path))
    suspicious_ips: dict[str, list[ForcedProof]] = {}
    keyed_windows: dict[tuple[str, str], deque[RequestRow]] = {}
    keyed_ip_counts: dict[tuple[str, str], Counter[str]] = {}
    keyed_ua_family_counts: dict[tuple[str, str], Counter[str]] = {}
    ...
    for row in sorted_rows:
        ...
        window = keyed_windows.setdefault(key, deque())
        ip_counts = keyed_ip_counts.setdefault(key, Counter())
        ua_family_counts = keyed_ua_family_counts.setdefault(key, Counter())

        window.append(row)
        ip_counts[row.ip] += 1
        ua_family_counts[ua_family(row.ua)] += 1
        ...
        while window and window[0].ts < cutoff:
            ...
            ua_family_counts[expired_family] -= 1
            if ua_family_counts[expired_family] <= 0:
                del ua_family_counts[expired_family]

        window_len = len(window)
        if window_len < args.low_context_fanout_count:
            continue
        unique_ips = len(ip_counts)
        if unique_ips < args.low_context_fanout_unique_ips:
            continue
        max_ip_share = max(ip_counts.values()) / window_len if window_len else 1.0
        if max_ip_share > args.low_context_fanout_max_ip_share:
            continue
        dominant_family, dominant_family_count = ua_family_counts.most_common(1)[0]
        ...
        detail = (
            f"path={path} ref={ref_key} ips={unique_ips} reqs={window_len} "
            f"family={dominant_family} family-dom={dominant_family_ratio:.0%} "
            f"window={compact_ts(window[0].raw_ts)}..{compact_ts(window[-1].raw_ts)}"
        )
        for suspicious_ip in ip_counts:
            suspicious_ips.setdefault(suspicious_ip, []).append(
...
def analyze_ip_behaviors(
    ...
    # Low-context revisit detector: repeated fetches of one content target with
    # only direct/root referer context are weak evidence per request, but strong
    # evidence when the same IP keeps doing it across a meaningful span.
    low_context_rows: dict[tuple[str, str], list[RequestRow]] = {}
    for row in rows:
        ...
        if ref_key is None:
            continue
        low_context_rows.setdefault((base_path(row.path), ref_key), []).append(row)
    for (path, ref_key), path_rows in low_context_rows.items():
        if len(path_rows) < args.low_context_revisit_count:
            continue
        path_rows.sort(key=lambda row: row.ts)
        span = path_rows[-1].ts - path_rows[0].ts
        if span < args.low_context_revisit_min_span_seconds:
            continue
        proofs.append(
            ForcedProof(
                kind="low-context-revisit",
                detail=(
                    f"path={path} ref={ref_key} repeats={len(path_rows)} "
                    f"span={span:.0f}s window={compact_ts(path_rows[0].raw_ts)}.."
                    f"{compact_ts(path_rows[-1].raw_ts)}"
                ),
            )
        )
        break
...
def print_report(
    ...
    low_context_fanout_analysis: CoordinatedUAAnalysis,
    ...
    if (
        ...
        or low_context_fanout_analysis.degraded
        ...
        if low_context_fanout_analysis.degraded:
            print(
                "  low-context-fanout: degraded "
                f"({low_context_fanout_analysis.degraded_reason})"
            )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        type=float,
        default=21600.0,
        help="Window for distributed low-context same-target fanout detection.",
    )
    parser.add_argument(
        "--low-context-fanout-count",
        type=int,
        default=5,
        help="Low-context same-target requests before flagging distributed fanout.",
    )
    parser.add_argument(
        "--low-context-fanout-unique-ips",
        type=int,
        default=5,
        help="Distinct IPs required for low-context same-target fanout detection.",
    )
    parser.add_argument(
        "--low-context-fanout-max-ip-share",
        type=float,
        default=0.35,
        help="Largest single-IP share allowed in a low-context fanout window.",
    )
    parser.add_argument(
        "--low-context-fanout-max-ip-requests",
        type=int,
        default=2,
        help="Only include IPs with no more than this many scoped requests in low-context fanout.",
    )
    parser.add_argument(
        "--low-context-fanout-min-ua-family-ratio",
        type=float,
        default=0.7,
        help="Dominant UA-family ratio required for low-context fanout detection.",
    )
    parser.add_argument(
        "--low-context-fanout-max-rows",
        type=int,
        default=250000,
        help="Max candidate rows to scan for low-context fanout detection.",
    )
    parser.add_argument(
        "--pair-gap-seconds",
        type=float,
    ...
    parser.add_argument(
        ...
        type=int,
        default=8,
        help="Same low-context content target requests from one IP before flagging.",
    )
    parser.add_argument(
        "--low-context-revisit-min-span-seconds",
        type=float,
        default=300.0,
        help="Minimum span required for same-IP low-context revisit detection.",
    )
    parser.add_argument(
...
def main() -> int:
    ...
    try:
        ...
        for ip, proofs in target_fanout_analysis.forced_proofs_by_ip.items():
            forced_proofs.setdefault(ip, []).extend(proofs)
        low_context_fanout_analysis = detect_low_context_fanout(analysis.rows, args)
        for ip, proofs in low_context_fanout_analysis.forced_proofs_by_ip.items():
            forced_proofs.setdefault(ip, []).extend(proofs)
